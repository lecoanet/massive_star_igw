...
mesa_file = '../10/profile1.data.GYRE'
...
for (start,end) in zip(start_list,end_list):
  f_list.append(np.linspace(start,end,13,endpoint=False)[1:])

f_list = np.array([f for sublist in f_list for f in sublist])

f_list = 10**(f_list)

for f in f_list[rank::size]:
  ...
  if not os.path.exists('done/%s' %output_folder):
  ...
  #  problem.add_bc("right(p) = 0")
    ...
    try:
        ...
        file = open('done/%s' %output_folder, 'w')
        file.close()
        end_time = time.time()
